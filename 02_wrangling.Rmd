...
## We now replace missing values for the numerical variables by the 
...
penguins_imputed <- penguins |>
  ...
  mutate(imputed = if_any(where(is.numeric), is.na),
         across(where(is.numeric), mean_impute)) |> ungroup() |> 
...
```
--
If we now extract the standard deviation from the above data, we get
```{r error = TRUE}
...
---
The warning message suggests that we use the [reframe](https://dplyr.tidyverse.org/reference/reframe.html) function.
```{r}
pop_dest |> reframe(delay_quantile = quantile(arr_delay))
```
...
```{r error = TRUE}
pop_dest |> ungroup() |> 
  reframe(delay_quantile = quantile(arr_delay))
...
```{r}
pop_dest |> 
  reframe(val = quantile(arr_delay), q = c(0, 0.25, 0.5, 0.75,1))
