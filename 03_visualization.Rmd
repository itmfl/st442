...
---
class: clear
A **ggplot2** version is below; the code is from Andrew Heiss [blog post](https://www.andrewheiss.com/blog/2017/08/10/exploring-minards-1812-plot-with-ggplot2/).
...
We now present two of the most elementary and widely used plots.
...
p1 <- ggplot(SAT_2010, aes(x = math)) + geom_histogram(binwidth = 10)
p2 <- ggplot(SAT_2010, aes(x = math, after_stat(density))) + 
...
```{r cache = TRUE, out.width="70%", error = TRUE, fig.asp = 0.5}

ggplot(data = head(SAT_2010,10), aes(x = state, y = math)) + xlab("") + 
...
Now compare the following codes.
```{r cache = TRUE, out.width="70%", fig.asp = 0.5}
p <- ggplot(data = head(SAT_2010,10), aes(x = state)) + 
  xlab("") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
p1 <- p + geom_col(aes(y = math))
p <- ggplot(data = head(SAT_2010,10), aes(x = reorder(state, math))) + 
  xlab("") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
p2 <- p + geom_col(aes(y = math))
grid.arrange(p1,p2,ncol = 2)
```
---
class: clear
...
Now compare the following codes
```{r cache = TRUE, out.width = "70%", fig.asp = 0.5}
p <- ggplot(data = head(SAT_2010,10), aes(x = reorder(state, math), y = math)) +
  xlab("") + geom_col() 
...
p2 <- p + scale_y_reverse() + coord_flip()
grid.arrange(p1,p2,ncol = 2)
```
--
...
knitr::include_graphics("https://hugelolcdn.com/i/123968.gif")
```
...
---
#Bar plots vs dot plots ?
The plot on the right is known as a Cleveland dot plot after [William Cleveland](https://en.wikipedia.org/wiki/William_S._Cleveland). The dot plot is quite a bit less "noisy" compared to the bar plot.
```{r cache = TRUE, out.width = "70%", fig.asp = 0.5}
p <- ggplot(data = head(SAT_2010,10), 
            aes(x = reorder(state, math), y = math)) + xlab("")
p1 <- p + geom_col() + coord_flip() + theme_bw()
p2 <- p + geom_point(size = 3) + coord_flip() + theme_bw()
grid.arrange(p1,p2,ncol = 2)
```
---
#Polished graphics
...
This example tries to reproduce a few representative figures/plots from the article *How to tell someone's age when all you know is her name* on the website [Five Thirty Eight](https://fivethirtyeight.com/features/how-to-tell-someones-age-when-all-you-know-is-her-name/). The example follows section 3.3 of the book *Modern Data Science with R* and uses data from the **babynames** package; this package contains information about roughly $1.6$ million names during the period from $1900$ to $2010$. 

```{r cache = TRUE, echo = TRUE}
library(mdsr); library(babynames)
...
```{r cache = TRUE, fig.retina = 3, out.width = "80%", echo = FALSE}
...
We then compute the median age of people currently alive and are named Joseph. 
```{r median_yob, cache = TRUE, eval = FALSE}
library(dplyr)
median_yob <- joseph %>% 
  group_by(year) %>% 
  summarise(count = sum(est_alive_today)) %>% 
  mutate(prop = cumsum(count)/sum(count)) %>%
  filter(prop >= 0.5) %>% ## a binary search is more efficient
  arrange(year) %>% select(year) %>% pull() %>% first()
median_yob
```
```{r echo = FALSE, ref.label='median_yob'}
library(dplyr)
median_yob <- joseph %>% 
  mutate(prop = cumsum(est_alive_today)/sum(est_alive_today)) %>%
  filter(prop >= 0.5) %>% ## a binary search is more efficient
  arrange(year) %>% select(year) %>% pull() %>% first()
median_yob
```
...
```{r echo = FALSE, cache = TRUE, out.width="60%", fig.asp = 1.2}
...
com_fem <- BabyNamesDist %>% filter(sex == "F", name %in% com_fem_names$name) %>%
  ...
  summarise(est_num_alive = sum(est_alive_today),
            q1_age = Hmisc::wtd.quantile(age_today, est_alive_today, .25),
            median_age = Hmisc::wtd.quantile(age_today, est_alive_today, .5),
            q3_age = Hmisc::wtd.quantile(age_today, est_alive_today, .75)) %>%
  arrange(desc(est_num_alive)) %>% head(25)
...
---
class:clear
We then compute the $25$th percentile, the median, and the $75$th percentile for the current age of people with these names. 
```{r eval = FALSE}
library(Hmisc)
com_fem <- BabyNamesDist %>% 
  filter(sex == "F", name %in% com_fem_names$name) %>%
  group_by(name) %>%
  summarise(est_num_alive = sum(est_alive_today),
            q1_age = wtd.quantile(age_today, est_alive_today, .25),
            median_age = wtd.quantile(age_today, est_alive_today, .5),
            q3_age = wtd.quantile(age_today, est_alive_today, .75)) %>%
  arrange(desc(est_num_alive))
```
A snippet of the resulting data is given below.
```{r}
head(com_fem)
```
...
```
---
# Example 3: Weather trends
```{r cache = TRUE, echo = FALSE, out.width="100%", message = FALSE, warning = FALSE}
