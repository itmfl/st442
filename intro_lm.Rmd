...
We denote this as $\bm{x} \sim \mathrm{MVN}(\bm{\mu}, \boldsymbol{\Sigma})$.
...
# Transformations
```{r}
tetrahymena <- read.csv2("https://bit.ly/2ngMtJE", sep = ";", 
                         dec = ".", header=TRUE, 
                         colClasses = c("numeric","numeric","numeric"), na.strings=".")

tetrahymena <- subset(tetrahymena, glucose==0)
...
---
class: clear
```{r message = FALSE, echo = -c(1:3)}
library(sjlabelled)
library(sjmisc)
library(sjPlot)
m1 <- lm(diameter ~ concentration, data = tetrahymena)
m2 <- lm(diameter ~ log(concentration), data = tetrahymena)
summary(m1)$sigma^2 ## MSE for model 1
summary(m2)$sigma^2 ## MSE for model 2
```