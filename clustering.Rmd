...
There are two important issues in using hierarchical clustering. 
...
We now how scaling the data could impact the clustering.
```{r}
...
---
class: clear
Suppose we want to cluster the members of the Scottish parliement based on their voting records. Our approach is to first converting the data to a wide format, then do PCA, and then clustering the data via e.g., $K$-means.
...
clusters <- kmeans(voters, centers = 6, nstart = 50,iter.max = 50)
```
---
class: clear
...
Two important alternatives to $K$-means clustering are
