...
+ dimension reduction: e.g., PCA, low-rank matrix recovery, classical multidimensional scaling.
...
on the fuel economy rating of cars from the US Department of Energy. We follow the exposition in Chapter 12 of Baumer et al. [Modern Data Science with R 2nd edition](https://beanumber.github.io/mdsr2e/ch-learningII.html#clustering)
...
library(tibble)
library(ggplot2)
filename <- "data/2026_feguide.xlsx"
cars <- read_excel(filename) %>% 
  ...
  select(make, model, displacement, number_cyl, 
         number_gears, city_mpg, hwy_mpg) |>
...
class: clear
```{r eval = FALSE}
plot(cex = 0.6, hang = -1, xlab = "", sub = "", 
     main = "Hierarchical clustering of Toyota cars (complete linkage)")
```
```{r echo = FALSE, out.height="100%"}
knitr::include_graphics("figures/toyota_clustering.png")
```

--

Good luck reading those labels. 
...
blog_adj <- as_adjacency_matrix(blog_graph, sparse=FALSE) ## Get adjacency matrix
dim(blog_adj)
image(blog_adj)
```
...
If we do not transform the embedding prior to $K$-means then we get
```{r}
clusters <- kmeans(X, centers = 2, nstart = 50)
table(clusters$cluster, V(blog_graph)$color) ## error rate of roughly 31%
```