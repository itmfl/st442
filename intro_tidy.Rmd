...
In the previous examples, **gather** and **spread** are inverse transformations. But this is not always the case. For example
```{r}
stocks <- tribble(
  ~year, ~half, ~return,
  2015, 1, 1.88,
...
---
# The joy of join()
To go from table $4a$ and $4b$ to Table $1$, we need to combine columns. More specifically
.pull-left[
```{r eval = FALSE}
...
```
```{r echo = FALSE}
knitr::kable(table4b %>% gather(key = "year", value = "cases", `1999`, `2000`) %>% 
  arrange(country, year)) %>% kable_styling(bootstrap_options = "striped", font_size = 14)
```
]
---
class: clear
We then *join* the columns of the *reshaped* table $4b$ to that of the reshaped $4a$
```{r echo = 3, eval = -3}
...
knitr::kable(table4a.long %>% left_join(table4b.long)) %>%
  kable_styling(bootstrap_options = "striped", font_size = 14)
```