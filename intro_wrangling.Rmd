...
---
class: clear
There are a large number of useful summary functions. We provide a few more examples.
```{r}
## average arrival delay
not_canceled <- filter(flights, !is.na(dep_delay) & !is.na(arr_delay))
not_canceled %>% group_by(year, month, day) %>% 
  summarize(avg_delay = mean(arr_delay), avg_positive_delay = mean(arr_delay[arr_delay > 0]))
```
---
class: clear
```{r}
