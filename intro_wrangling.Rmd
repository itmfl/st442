...
```
To get multiple summaries statistics for each column, we pass the functions as a list.
```{r}
...
```{r}
df %>% summarise_all(~max(.)/min(.)) ## Pass an inline anonymous function.
...
df %>% summarise_at(vars(-z), 
         list(min = min, max = max))
df %>% summarise_at(vars(contains("x")), 
         median)
```
]
---
...
```{r}
starwars
starwars %>% summarize_if(is.numeric, list(mean = mean, sd = sd), na.rm = TRUE)
```
---
class: clear
...
---
class: clear
If we now extract quantiles from the above data frame/vector, we get
```{r error = TRUE}
flights %>% select(arr_delay) %>% quantile(na.rm = TRUE)
flights %>% pull(arr_delay) %>% quantile(na.rm = TRUE)
```

The difference in the above output is due to the fact that $\mathrm{quantile}$ expects a *numeric* vector. Meanwhile, the *select* verb takes as input a *data frame* and ouputs another *data frame*. Since we want to extract the column $\mathrm{arr\_delay}$ ,we need to use $\mathrm{pull}$. 
---
#summarise and do
