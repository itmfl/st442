...
subtitle: "An introduction to dplyr (part I)"
author: "CSC/ST 442"
date: "Fall 2019"
output:
  xaringan::moon_reader:
    css: [mystyle.css, default, xaringan-themer.css]
    lib_dir: libs
    nature:
...
options(htmltools.dir.version = FALSE,digits=3,tibble.print_min=6)
```
...
---
We shall follow an an abbreviated presentation from Chapter 5 and 7 of [R for data science](https://r4ds.had.co.nz/). We first load the *flights13* dataset from the **nycflights13** package. This dataset describe the on-time data for all flights departing from NYC airports (i.e. JFK, LGA or EWA) in 2013.
...
```
---
The previous slide introduce the use of the $\%>\%$ pipe operator. 
...
The pipe operator maps the command $x \%>\% f(y)$ to $f(x,y)$; similarly, $x \%>\% f(y) \%>\% g(z)$ is mapped to $g(f(x,y),z)$. 
---
##Missing values
...
```
---
There are a large number of useful summary functions. We provide here a few more examples.
```{r}
...
group_by(not_canceled, dest, month) %>% 
  ...
  summarize(air_time = mean(air_time)) %>% head(5)
...
group_by(not_canceled, dest, month) %>% 
  ...
  summarize(air_time = median(air_time)) %>% head(6)
group_by(not_canceled, dest) %>% 
  summarize(air_time = median(air_time)) %>% head(6)
```
---
...
```
---
Functions that work most naturally in grouped mutates and grouped filters are [window functions](https://rpubs.com/hadley/window) (see also the **R** command $\mathrm{vignette("window-functions")}$. 

The following example is from the link [window functions](https://rpubs.com/hadley/window). The example uses a dataset from the **Lahman** database that contains pitching, hitting, and fielding statistics for MLB from $1871$ through $2018$. We will consider a sample of batting data for players that have won an award.
```{r}
library(Lahman)
