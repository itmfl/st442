...
The *SVD* is the **crown jewel** of numerical linear algebra.
...
where $\boldsymbol{\Sigma}$ is a diagonal matrix with diagonal entries $\sigma_1 \leq \sigma_2 \leq \dots \leq \sigma_{m}$. Furthermore,
...
are the eigendecompositions of $\mathbf{X}\mathbf{X}^{\top}$ and $\mathbf{X}^{\top} \mathbf{X}$. 

+ Diagonal elements $\sigma_1, \dots, \sigma_m$ of $\boldsymbol{\Sigma}$ are the **singular values**. 
+ Columns of $\mathbf{U}$ and $\mathbf{V}$ are the **left** and **right** **singular vectors**.  

**Note**: If $n \leq m$ then $\mathbf{X} = \mathbf{U} \begin{bmatrix} \boldsymbol{\Sigma} & \boldsymbol{0} \end{bmatrix} \mathbf{V}^{\top}$ where $\boldsymbol{\Sigma}$ is a $n \times n$ diagonal matrix.

---
## SVD and low-rank approximation

**Theorem** (Eckart-Young-Mirsky (1936)) Let $\mathbf{X}$ be a $n \times m$ matrix with $n \geq m$. Write the SVD of $\mathbf{X}$ as $\mathbf{X} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^{\top}$. 
Let $k \leq m$. Then the **best** rank $k$ approximation (with respect to sum of square error) to $\mathbf{X}$ is the matrix
...
+ $\boldsymbol{\Sigma}_{k}$ is the diagonal matrix containing the first $k$ diagonal elements of $\boldsymbol{\Sigma}$. 

That is to say, for all $n \times m$ matrix $\mathbf{Z}$ with $\mathrm{rank}(\mathbf{Z}) = k$, we have
$$\sum_{i} \sum_{j} (x_{ij} - z_{ij})^2 \geq \sum_{i} \sum_{j} (x_{ij} - x_{k,ij})^2 = \sum_{\ell=k+1}^{m} \sigma_{\ell}^2.$$
...
plot(1:length(t), t/max(t), 
     xlab = "Number of dimensions included", ylab = "Approximation accuracy")
```
...
---
class: clear
```{r svd_fast1, depends_on="svd_fast0", cache = TRUE}
library(irlba)
r <- 5
ptm <- proc.time()
...
proc.time() - ptm
```
**irlba** has a speed up of roughly $30$ times. This speed up is even more pronounced for larger matrices. The error between the two methods is minimal.
```{r}
X_approx1 <- X_svd$u %*% diag(X_svd$d[1:r]) %*% t(X_svd$v)
X_approx2 <- X_irlba$u %*% diag(X_irlba$d[1:r]) %*% t(X_irlba$v)
sum((X_approx1 - X_approx2)^2)/sum(X_approx1^2) ## Relative error
```
...
```{r mnist1, cache = TRUE}
mnist_train <- read.csv("data/mnist_train.csv", 
...
---
class: clear
```{r mnist4, cache = TRUE, warning = FALSE}
library(kernlab)
XY_lift <- tibble::as_tibble(cbind(train$label, X_lift))
names(XY_lift)[1] <- "label"
ksvm(factor(label) ~ ., data = XY_lift, 
     kernel = "vanilladot", scaled = TRUE, C = 10)
```
...
Principal component analysis (PCA) is a **special** case of SVD. 

Let $\mathbf{X}$ be a $n \times p$ matrix whose rows are observations and columns are variables. Given an integer $d \leq p$, the **classical** motivation for PCA 
...
However, to make this criterion precise (more specifically, in order to define the notion of variability precisely) requires quite a bit more setup. We thus do not pursue this motivation of PCA here.

---
class: clear

Our motivation for PCA is simply as follows. We want to find a $n \times d$ matrix $\mathbf{Z}$ such that the **columns** of $\mathbf{X}$ can be reconstructed from linear combinations of the columns of $\mathbf{Z}$ with **minimum** error. 
...
The solution is thus once again given by SVD. More specifically
...
+ (Optional) Scale $\tilde{\mathbf{X}}$ to have (column-wise) sample variance equal to $1$.  
+ Compute SVD of $\tilde{\mathbf{X}}$ as $\tilde{\mathbf{X}} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^{\top}$.
...
+ Recall that $\mathbf{U} \boldsymbol{\Sigma}^2 \mathbf{U}^{\top}$ is the **eigendecomposition** of the $n \times n$ matrix $\tilde{\mathbf{X}} \tilde{\mathbf{X}}^{\top}$.

+ Similarly, $\tilde{\mathbf{V}} \boldsymbol{\Sigma}^2 \tilde{\mathbf{V}}^{\top}$ is the **eigendecomposition** of the $p \times p$ matrix $\tilde{\mathbf{X}}^{\top} \tilde{\mathbf{X}}$.
...
+ Given a new $X_*$ in $\mathbb{R}^{p}$, we map $X_*$ into $Z_* \in \mathbb{R}^{d}$ (the PCA space) via
$$Z_* = \mathbf{V}_d^{\top} (X_* - \bar{\boldsymbol{x}})$$

---
## PCA in R

PCA in R is done via either the function **prcomp** or **princomp**. 

From the help page for **prcomp**
...
iris_centered <- scale(iris_X, 
                      center = TRUE, 
                      scale = FALSE) 
iris_svd <- svd(iris_centered)
## The loading matrix
iris_svd$v 
```
]
...
It turns out that this problem is, once again, intrinsically related to SVD
...
A related but easier optimization problem is **classical multidimensional scaling**.
...
Now suppose, without loss of generality, that $\sum_{i} X_i = \boldsymbol{0}$.

Then $$m_{ij} := \delta_{ij}^2 - \frac{1}{n} \sum_{i} \delta_{ij}^2 - \sum_{j} \delta_{ij}^2 + \frac{1}{n} \sum_{i} \sum_{j} \delta_{ij}^2 = - 2 X_i^{\top} X_j$$
...
library(mdsr)
world_cities
BigCities <- world_cities |> arrange(desc(population)) |> head(4000) |>
...
---
## Matrix completion and iterative SVD.
...
## Mean square error of recovery
norm(D - D.reconstruct, type = "F")^2/length(D) 
## Mean absolute error of recovery
mean(abs(D - D.reconstruct)) 
## Relative error of recovery
