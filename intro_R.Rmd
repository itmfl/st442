...
+ effective tools for **literate programming**, e.g., creating a report that seamlessly weave code/data together with formulas, texts, documentation. In fact, these slides are written in [Rmarkdown](https://rmarkdown.rstudio.com/).
...
+ Long and Teetor [R Cookbook, 2nd edition](https://rc2e.com/)

+ Garrett Grolemund and Hadley Wickham, [R for Data Science, 2nd edition](https://r4ds.hadley.nz/)

+ Benjamin Baumer, Daniel Kaplan, and Nicholas Horton, [Modern Data Science with R, 2nd edition](https://beanumber.github.io/mdsr2e/)
...
---
class: clear
You can use mutate with the `.keep="none"` option to drops all variables except the newly created variables, e.g.,
```{r}
mutate(flights, gain = arr_delay - dep_delay, 
                   hours = air_time/60, 
                   gain_per_hour = gain/hours, .keep="none")
```
---
#summarize()
...
The previous examples uses the $.$ operator, which serves as a placeholder for the variable/data of interest. 

It is, however, generally cleaner (syntax wise), to define and use a named function as opposed to an anonymous function.
...
flights |> select(arr_delay) |> class()
```
---
class: clear
```{r error = TRUE}
library(dplyr)
flights |> dplyr::pull(arr_delay) |> head()
flights |> pull(arr_delay) |> class()
```
--
