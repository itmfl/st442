...
---
class: clear
As another example of a column of a data frame being a vector, we note that a matrix is an attributed vector, e.g.,
...
Using the **dplyr** and **tidyr** approach we can create list columns in a data frame via three widely applicable methods
...
nested
## Equivalently
## nested <- gapminder %>% nest(-country,-continent)
## nested <- gapminder %>% nest(year:gdpPercap)
```
]
.pull-right[
```{r}
